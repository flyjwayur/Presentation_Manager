...
const presentationReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    const indexOfPresentation = state.presentations.findIndex(presentation => 
      presentation._id === presentationId);
    ...
    console.log({
      ...
      presentations : [
        ...
        ...state.presentations.slice(indexOfPresentation + 1, state.presentations.length)
      ]
    });
    return {
      ...state,
